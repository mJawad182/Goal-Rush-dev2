    ...
    public class ShootingBehaviour : AbstractShootingBehaviour {
        ...
        private float toleranceMod {
            get {
                ...
                if (opponentGK.GameTeam.GamePlayers.Length <= 1)
                    return default;
        ...
        public override bool Behave(bool isAlreadyActive) {
            ...
            if (isAlreadyActive) {
                ...
                if (Player.LookTo(in deltaTime, shootingDir)) {
                    ...
                    Debug.Log("FINAL BEFORE SHOOT = " + target);

                    // FINAL override shot strength
                    float slowShotMultiplier = 0.05f;   // ← tiny value = very slow shot
                    target *= slowShotMultiplier;

                    // Debug AFTER scaling
                    Debug.Log("FINAL AFTER SLOW = " + target);
                    Player.Shoot(target);
                }
