...
namespace FStudio.MatchEngine.Balls {
    ...
    public partial class Ball : SceneObjectSingleton <Ball> {
        private float finalVelocityMultiplier = 0.8f; // 1 = normal, <1 = slower, >1 = faster
        ...
        public void Pass (
            ...
            if(HolderPlayer.GameTeam.GamePlayers.Length <= 1)
            {
                return;
            }
        ...
        private void BallHit (
            ...
            //// Apply final multiplier (configurable in inspector)
            velocity *= finalVelocityMultiplier;

            Debug.LogFormat("[Ball] Velocity AFTER finalMultiplier => {0}", velocity);
