...
namespace FStudio.MatchEngine {
    public class MatchManager : SceneObjectSingleton<MatchManager> {
        ...
        void OnDrawGizmos()
        {
            ...
            // 1. Draw the outer field rectangle (using fieldEndX, fieldEndY) on the XZ plane.
            // 2. Use a distinct color (green) for the field outline so it's visible in the editor.
            // 3. Draw the halfway (center) line vertically across the field.
            // 4. Draw a center circle (wire sphere) to mark the kickoff area.
            // 5. Draw the two goal net wire cubes (existing goalNet1Rect / goalNet2Rect) in a different color.
            // 6. Slightly raise all gizmos on Y so they are visible above the ground plane.
            //
            // Notes:
            // - Coordinates assume field origin at (0,0,0) and field extents along +X and +Z up to fieldEndX, fieldEndY.
            // - Keep gizmo Y offset small (0.01f) to avoid z-fighting with ground.
            //
            // Implementation follows steps above.

            // Early exit if field sizes are invalid to avoid drawing nonsense.
            if (fieldEndX <= 0 || fieldEndY <= 0)
            {
                // Still draw goals if their rects are meaningful.
            ...
            // Field corners (XZ plane)
            var bl = new Vector3(0f, yOffset, 0f);                      // bottom-left (near, left)
            var br = new Vector3(fieldEndX, yOffset, 0f);               // bottom-right (near, right)
            var tr = new Vector3(fieldEndX, yOffset, fieldEndY);       // top-right (far, right)
            var tl = new Vector3(0f, yOffset, fieldEndY);              // top-left (far, left)
            ...
            // Draw halfway (center) line
            Gizmos.color = Color.white;
            var centerLeft = new Vector3(fieldEndX / 2f, yOffset, 0f);
            var centerRight = new Vector3(fieldEndX / 2f, yOffset, fieldEndY);
            Gizmos.DrawLine(centerLeft, centerRight);

            // Draw center circle for kickoff (wire sphere)
            Gizmos.color = Color.white;
            var center = new Vector3(fieldEndX / 2f, yOffset, fieldEndY / 2f);
            // radius approximates typical soccer center circle (adjust as needed)
            Gizmos.DrawWireSphere(center, Mathf.Min(fieldEndX, fieldEndY) * 0.08f);

            // Optional: draw small cross at center
            float crossSize = Mathf.Min(fieldEndX, fieldEndY) * 0.02f;
            Gizmos.DrawLine(center + new Vector3(-crossSize, 0, 0), center + new Vector3(crossSize, 0, 0));
            Gizmos.DrawLine(center + new Vector3(0, 0, -crossSize), center + new Vector3(0, 0, crossSize));

            // Draw goal net bounding boxes (keep original behavior)
            Gizmos.color = Color.yellow;
            Gizmos.DrawWireCube(
                new Vector3(goalNet1Rect.position.x, goalNet1Rect.height / 2f, goalNet1Rect.position.y),
                new Vector3(1f, goalNet1Rect.height, goalNet1Rect.width));

            Gizmos.DrawWireCube(
                new Vector3(goalNet2Rect.position.x, goalNet2Rect.height / 2f, goalNet2Rect.position.y),
                new Vector3(1f, goalNet2Rect.height, goalNet2Rect.width));
        }
        ...
        private async void StartKickoffCounter() {
            ...
            //Current.MatchFlags = MatchStatus.Freeze;
        ...
        private void KickOff (KickOffEvent _) {
            MatchFlags = MatchStatus./*WaitingForKickOff*/Playing;
        }
...
#region GK Condition
        ...
        [ContextMenu("Force Reshot")]
        private void ForceReShot()
        {
            matchSceneManager.ForceEndCelebration();
        }
                ...
                void reOrderTeam(
                    ...
                    switch (foulType) {
                        ...
                        case FoulType.GoalKick: 
                            teamBehaviour = !hasTheBall ? TeamBehaviour./*WaitingForOpponentGK*/OpponentThrowIn : TeamBehaviour.WaitingForTeamGK; 
                            break;
                        default: 
